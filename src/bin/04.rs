...
fn main() -> Result<()> {
    ...
    fn part1<R: BufRead>(reader: R) -> Result<usize> {
        ...
        let is_tail =
            |ch: char| ch == word.chars().nth(0).unwrap() || ch == l_char(&word.to_string());
        ...
        for (i, line) in reader.lines().flatten().enumerate() {
            ...
            while n < chars.len() {
                ...
                while c < word.len() {
                    if is_tail(chars[n]) && chars.len() - n >= word.len() {
                        buf[0].push(chars[n + c]);
                    }
                    if c <= i {
                        if (buf[1].len() <= 1 || !is_tail(l_char(&buf[1])))
                            && cache.contains_key(&(i - c, n))
                        {
                            buf[1].push(*cache.get(&(i - c, n)).unwrap());
                        }
                        if c <= n
                            && (buf[2].len() <= 1 || !is_tail(l_char(&buf[2])))
                            && cache.contains_key(&(i - c, n - c))
                        {
                            buf[2].push(*cache.get(&(i - c, n - c)).unwrap());
                        }
                        if c <= chars.len() - n
                            && (buf[3].len() <= 1 || !is_tail(l_char(&buf[3])))
                            && cache.contains_key(&(i - c, n + c))
                        {
                ...
                for (si, s) in buf
                    .iter()
                    .map(|buf_str| buf_str.chars().rev().collect::<String>())
                    .enumerate()
                {
            ...
            while n < chars.len() {
                ...
                if i > 0 && n > 0 {
                    if n < chars.len()
                        ...
                        && cache.contains_key(&(i - 1, n + 1))
                        && is_tail(*cache.get(&(i - 1, n - 1)).unwrap())
                        && is_tail(*cache.get(&(i - 1, n + 1)).unwrap())
                    {
                        cache.insert((i, n), chars[n]);
                    }
                    ...
                    && cache.contains_key(&(i - 2, n))
                {
                    ...
                    if is_middle(*upper_middle) && is_tail(*upper_left) && is_tail(*upper_right) {
                        ...
                        let second_word =
                            format!("{}{}{}", *upper_right, *upper_middle, chars[n - 2]);
                        if i == 2 && n == 3 {
                            println!("{} {}", first_word, second_word)
                        };
                        if is_equal(first_word) && is_equal(second_word) {
                            result += 1
                        }
                    }
                }
