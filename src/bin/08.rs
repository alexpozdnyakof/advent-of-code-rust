...
fn main() -> Result<()> {
    ...
    fn part1<R: BufRead>(reader: R) -> Result<usize> {
        ...
        for (y, line) in reader.lines().flatten().enumerate() {
            for (x, tag) in line.chars().enumerate() {
                if tag == '.' || tag == '#' { continue }
                let antenna_point = Point { x: x as isize, y: y as isize };
                if let Some(broadcast_list) = broadcasters.get_mut(&tag) {
                    for prev_antenna_point in broadcast_list.iter() {
                        ...
                        if in_field_range(&antinode, line.len()) { antinodes.insert(antinode); }

                        let antinode = get_bottom_antinode_position(&antenna_point, prev_antenna_point);
                        if in_field_range(&antinode, line.len()) { antinodes.insert(antinode); }
                    }
    ...
    fn part2<R: BufRead>(reader: R) -> Result<usize> {
        // на каждые координаты запускать поиск до конца поля
        // добавлять спаренные антенны 
        let mut broadcasters: HashMap<char, Vec<Point>> = HashMap::new();
        let mut antinodes: HashSet<Point> = HashSet::new();
        for (y, line) in reader.lines().flatten().enumerate() {
            for (x, tag) in line.chars().enumerate() {
                ...
                if let Some(broadcast_list) = broadcasters.get_mut(&tag) {
                    ...
                    for prev_antenna_point in broadcast_list.iter() {    
                        ...
                        let mut antinode = get_up_antinode_position(&antenna_point, prev_antenna_point);
                        ...
                        while in_field_range(&antinode, line.len()) { 
                            antinodes.insert(antinode);
                            let new_antinode = get_bottom_antinode_position(&antinode, &prev_antinode);
                ...
                } else { broadcasters.insert(tag, vec![antenna_point] ); }
