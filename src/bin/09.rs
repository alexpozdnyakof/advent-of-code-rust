...
impl LocationIndex {
    ...
    fn insert(&mut self, position: usize, descriptor: LocationDescriptor) {
        ...
        self.index.insert(new_start, LocationDescriptor { id: descriptor.id, size: new_size });
    }
...
fn from_raw(raw: &RawBlocks) -> NTFS {
    ...
    for (i, ch) in raw.chars().enumerate() {
        if let Some(size) = ch.to_digit(10).map(|n| n as usize) {
            if i % 2 == 0 {
                ntfs.files.index.insert(offset, create_data_desc(size, i / 2));
            } else {
...
impl NTFS {
    ...
    fn free(&mut self, target: usize, size: usize) -> Result<(usize, usize)> {
        if let Some((&file_position, file)) = self.files.index.range(..=&target).last() {
            ...
            if size == file.size{
                self.free_space.insert(file_position, create_free_desc(file.size));
                ...
                if let Some(file_id) = file.id {
                    ...
                    self.free_space.insert(file_position + new_file.size, free_space);
    ...
    fn copy(&mut self, file_position: usize, size: usize, allocated_position: usize) {
        if let Some(file) = self.files.index.get(&file_position) { 
            if file.size == size {
                ...
                if let Some(file_id) = file.id {
                    self.files.insert(allocated_position,  create_data_desc(size, file_id));
                }
            }
        }
    }
    ...
    fn check_sum(&self) -> usize {
        self.files.index.iter().fold(0, |mut acc, (&position, &file)| { 
    ...
    fn allocate_before(&mut self, max_position: &usize, size: usize) -> Option<usize> {
            for (&position, &free) in self.free_space.index.range(..=max_position) {
                if free.size >= size { 
                    ...
                    if free.size > size { self.free_space.insert(&position + size, create_free_desc(free.size - size)); }
    ...
    fn print(&self) {
       let mut e_index = self.files.index.clone();
        ...
        let chars: String = e_index.iter().flat_map(|(_,desc)| { vec![if let Some(id) = desc.id { id.to_string() } else { String::from('.') }; desc.size]}).collect();
        println!("{}", chars);
...
fn main() -> Result<()> {
    ...
    fn part1<R: BufRead>(reader: R) -> Result<usize> {
        ...
        if let Some(raw_blocks) = reader.lines().next().transpose()? {
            ...
            let files: Vec<(usize, LocationDescriptor)> = file_system.files.index.iter().map(|(&k,&v)| (k, v)).collect();
            let chunk_size = 1;
           'outer: for (position, file) in files.iter().rev() {
                ...
                while size != 0 {
                    if let Some(allocated_position) = file_system.allocate_before(position, chunk_size) {
                        ...
                        if file_system.free(*position, chunk_size).is_ok() {
                            size -= chunk_size;
                        } else {
