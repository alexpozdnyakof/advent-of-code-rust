...
fn main() -> Result<()> {
    ...
    fn part1<R: BufRead>(reader: R) -> Result<usize> {
        ...
        for line in reader.lines().flatten() {
             println!("{}", line.len());
             if line.len() == 0 { is_parsing_finished = true; continue; }
             if !is_parsing_finished {
                  let [v, e]:[usize; 2] = collect_array(line.split("|").map(|s| { s.parse::<usize>().unwrap()}));
                  graph.add_edge(v, e);
             } else {
                  let path:Vec<usize> = line.split(",").map(|s| { s.parse::<usize>().unwrap() }).collect(); 
